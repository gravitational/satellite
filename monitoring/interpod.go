...
func (r *interPodChecker) testInterPodCommunication(ctx context.Context, client *kube.Clientset) error {
	...
	svc, err := client.CoreV1().Services(testNamespace).Create(context.TODO(), &v1.Service{
	...
	cleanupService := func() {
		if err = client.CoreV1().Services(testNamespace).
			Delete(context.TODO(), svc.Name, metav1.DeleteOptions{}); err != nil {
	...
	cleanupPods := func() {
		for _, podName := range podNames {
			if err = client.CoreV1().Pods(testNamespace).Delete(context.TODO(), podName, metav1.DeleteOptions{}); err != nil {
...
func createNamespaceIfNeeded(client *kube.Clientset, namespace string) error {
	...
	if _, err := client.CoreV1().Namespaces().Get(context.TODO(), namespace, metav1.GetOptions{}); err != nil {
		...
		_, err = client.CoreV1().Namespaces().Create(context.TODO(),
			&v1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: namespace}}, metav1.CreateOptions{})
		if err != nil {
			return trace.Wrap(err)
		}
	}
